...
public sealed class FullscreenOverlayService
{
    ...
    public void EnsureProfile(LauncherSettings settings, bool updateBounds)
    {
        var size = GetOverlaySize(settings);
        EnsureAssets(size.Width, size.Height);
    ...
    public void WriteRuntime(LauncherSettings settings, string setupScript, string observerScript)
    {
        var size = GetOverlaySize(settings);
        EnsureAssets(size.Width, size.Height);
        var runtime = setupScript + Environment.NewLine + observerScript;
        var runtimePath = Path.Combine(CounterDirectory, "runtime.js");
        if (File.Exists(runtimePath) && File.ReadAllText(runtimePath) == runtime) return;
    ...
    private static void EnsureAssets(int width, int height)
    {
        ...
        if (!File.Exists(runtimePath)) File.WriteAllText(runtimePath, "(function(){})();", new UTF8Encoding(false));
        if (!File.Exists(versionPath)) File.WriteAllText(versionPath, "0", new UTF8Encoding(false));
    }
...
<style>html,body{width:100%;height:100%;margin:0;overflow:hidden;background:transparent}#mma-source{display:block;width:100%;height:100%;border:0;background:transparent}</style>
</head><body><iframe id="mma-source" src="/ManiaMapAnalyser/?launcher-fullscreen=1" allowtransparency="true"></iframe>
<script>(function(){const frame=document.getElementById('mma-source');const base='/ManiaMapAnalyzerOverlay/';let appliedVersion='';
async function apply(version){try{const doc=frame.contentDocument;if(!doc||!doc.head)return;const previous=doc.getElementById('mma-fullscreen-runtime');if(previous)previous.remove();const script=doc.createElement('script');script.id='mma-fullscreen-runtime';script.src=base+'runtime.js?v='+encodeURIComponent(version);doc.head.appendChild(script);appliedVersion=version;}catch(_){}}
async function refresh(){try{const response=await fetch(base+'runtime.version?t='+Date.now(),{cache:'no-store'});const version=(await response.text()).trim();if(version&&version!==appliedVersion)await apply(version);}catch(_){}}
