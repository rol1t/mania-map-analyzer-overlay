...
## Restart checkpoint — 2026-08-22 (uncommitted working tree)

> Concise restart checkpoint appended without modifying source, running commands, or committing. All work described below remains **uncommitted/unstaged** on branch `refactor/maintainability-checkpoint` (ahead of `origin/main`). No commit/push is implied. Added adjacent to the maintainability checkpoint / manual-acceptance area for easy resume after an OpenCode restart.

### Branch / current state

- Working tree is **uncommitted and unstaged**; 12 modified + 8 untracked files (see file list below). No staging or commit was performed for these changes.
- This section only updates `TODO.md`; source files and generated artifacts were not touched by this checkpoint edit.

### Explicit drag redesign (replaces failed native WM move-loop attempts)

- Replaced native `WM_NCLBUTTONDOWN`/`SendMessage` move-loop approaches with an explicit browser → C# protocol: `overlay:drag:start/move/end` carrying `screenX/screenY` (CSS pixels), `gestureId`, `pointerId`, `sequence`.
- Browser (`assets/overlay/runtime/host.js`): pointer capture via `setPointerCapture`/`releasePointerCapture`, `pointerdown/move/up/cancel` (fallback `mousedown/mousemove/mouseup`), deduped `gestureId`/`sequence`, stale `osuFocused` release, cancellation on `blur`/deactivation/click-through; mixed-DPI scaling is captured at gesture start and not recomputed mid-gesture.
- Host (`src/Avalonia/Platform/OverlayDragSession.cs` + `src/Avalonia/Views/MainWindow.axaml.cs` + `src/Avalonia/Platform/WindowsOverlayController.cs`): new `OverlayDragSession` — `Start`/`TryMove`/`End`/`Cancel`, Position-only updates (`Position = PixelPoint`), no `SizeToContent` during drag, resize exclusively through explicit 8 handles (`overlay:resize:<dir>` → `BeginResize` with `WmNcLButtonDown` edge hit-test).
- Native edge `WM_NCHITTEST` now always returns `HTCLIENT` while `overlayMode` is active, so ordinary card position never enters a native move/resize loop; only handle-initiated `SendMessage` can start a native resize. Guard-timer focus protection and stale-focus release remain via `SynchronizeWithOsuWindow`.

### Validation completed for latest drag implementation

- `dotnet format ManiaMapAnalyzerOverlay.sln --no-restore --verify-no-changes` — clean.
- `dotnet build ManiaMapAnalyzerOverlay.sln --configuration Release --no-restore` — succeeds.
- `dotnet test ManiaMapAnalyzerOverlay.sln --configuration Release --no-build` — passes: Core ~77 passed + 1 skipped, Replay 76 passed, Avalonia 64 passed; plus `node --check` on `host.js`/`renderer.js` and `git diff --check` pass.
- **Manual Windows/WebView runtime acceptance still required** — prior fixes passed automated tests but failed at runtime (drag stuck / clipped), so explicit Windows smoke test is mandatory before handoff.

### LN DAN fix

- Suppress `ln-dan` rank when LN percent is null or zero (`src/Avalonia/Services/HeadlessSnapshotConverter.cs` + `assets/overlay/runtime/renderer.js` `rankHasValue`/`hasLnPercent` gate); stale `ln-dan` is deleted from merged snapshot when current map has no long notes. Focused tests added in `tests/Avalonia.Tests/DanSnapshotTests.cs`.

### Pause coach work (uncommitted, runtime visual verification pending)

- Core: new snapshot contract `src/Core/Analysis/PauseCoachSnapshot.cs` (aggregate-only: `Timing`/`Performance`/`Section`/`Insights`/`Diagnostics`, `IsProvisional`), live telemetry models `src/ReplayAnalysis/LiveTelemetryFrame.cs`/`LiveJudgementCounts.cs`/`ApproximatePerformanceEvent.cs`, analyzer `src/ReplayAnalysis/PauseCoachAnalyzer.cs` with tests `tests/ReplayAnalysis.Tests/PauseCoachAnalyzerTests.cs`.
- Adapter: transition snapshot in `assets/analyzers/mania-map-analyser/adapter.js` (publishes `snapshot.pauseCoach` from live `play.score/accuracy/health/combo`, `beatmap.time.live`, `play.unstableRate`/`hitErrorArray`).
- Presentation: `assets/overlay/presets/companella-replay/template.html`/`style.css`, `assets/overlay/runtime/renderer.js` `renderPauseCoach`, `README.md` + localization updates; independent-review fixes applied.
- Limitation: provisional aggregate-only — no per-column/per-object/per-finger or LN-release claims; pending manual Companella Replay visual test.

### Known unresolved issues

- **5K/9K optional Etterna stage** logs `Unsupported keycount` as a user-visible error notification even though the headless engine returns `Partial` correctly; needs severity downgrade/suppression + dedicated test.
- **Worker crash / rebootstrap after navigation** may need investigation if reproducible (observed during pause-coach integration).

### Computer Use MCP (outside repo)

- Globally installed and configured outside the repository: `open-computer-use-mcp@0.3.1` (connected). Requires OpenCode restart to take effect. No credentials are stored in the repo. Configuration is global — do not add to repo-tracked files.

### Next actions after restart (in order)

1. Manual Windows smoke test: explicit drag/resize on Windows — inactive osu!, foreground protection, Alt-Tab, mixed-DPI (capture scaling at gesture start).
2. Fix any runtime drag issue based on logs; **do not return to native move loops**.
3. Fix unsupported-keycount notification severity (downgrade/suppress + test).
4. Manual pause-coach / Companella Replay visual test (provisional aggregate block, fidelity label).
5. Inspect complete diff (`git diff HEAD` + `git status --porcelain`) and rerun `dotnet format --verify-no-changes` / `dotnet build --configuration Release` / `dotnet test --configuration Release` before handoff.

### Currently modified / untracked files (grouped, generated files omitted)

- Modified (12): `README.md`; overlay/browser `assets/analyzers/mania-map-analyser/adapter.js`, `assets/overlay/runtime/host.js`/`renderer.js`/`interaction.css`, `assets/overlay/presets/companella-replay/template.html`/`style.css`; Avalonia/host `src/Avalonia/Platform/WindowsOverlayController.cs`, `src/Avalonia/Views/MainWindow.axaml.cs`, `src/Avalonia/Services/HeadlessSnapshotConverter.cs`; Core `src/Core/Analysis/AnalysisSnapshot.cs`; tests `tests/Avalonia.Tests/DanSnapshotTests.cs`.
- Untracked (8): `src/Avalonia/Platform/OverlayDragSession.cs`; Core/ReplayAnalysis `src/Core/Analysis/PauseCoachSnapshot.cs`, `src/ReplayAnalysis/PauseCoachAnalyzer.cs`, `src/ReplayAnalysis/LiveTelemetryFrame.cs`, `src/ReplayAnalysis/LiveJudgementCounts.cs`, `src/ReplayAnalysis/ApproximatePerformanceEvent.cs`; tests `tests/Avalonia.Tests/OverlayDragSessionTests.cs`, `tests/ReplayAnalysis.Tests/PauseCoachAnalyzerTests.cs`.
- This `TODO.md` update itself is an additional unstaged change; do not commit generated `analyzer-engines`/`node_modules` artifacts.
